...
<head>
	...
	$(document).ready(function() {
		$(".del_gallery_img").on('click',function(){
			var that = $(this);
			$.ajax({
				url: 'menu-main/menu-setting-submenu/wreaths/del_gallery_img.php',
				type: 'POST',
				dataType: 'html',
				data: {
					id: that.attr('id'),
					swreath_id: $("#special_wreath_id").val()
				},
		...
		<table id="gallery" style="border-style:solid; border-left-width:15px; border-color: #509690; margin-top: 5px; width: 730px;">
			...
			<tr>
				...
				<td id="gallery_images">
					...
					?> <input id="special_wreath_id" type="hidden" value="<?=$id ?>"> <?php
						$query = "SELECT id, url FROM `special_wreath_img` WHERE special_wreath_id = $id";
						$result = mysql_query($query) or die (mysql_error());
						while ($row = mysql_fetch_assoc($result)) {
