...
<td>
 <select id="<?php echo "color".$flowernum; ?>" name="<?php echo "color".$flowernum; ?>" onChange="calculatePrice_wForm(); writeFPrice();" disabled>
	<option disabled value="" selected>Először válasszon virág típust!</option>
 </select>
</td>

<td style="position: relative; left: -5px;"> 
	<input type="text" class="db" id="<?php echo "qty".$flowernum; ?>" name="<?php echo "qty".$flowernum; ?>" max="99" min="1" value="1" onChange="calculatePrice_wForm(); writeFPrice();" > db
</td>
<td style="position: relative; left: -45px;">
	<input type='button' class="inc_button" name='add_flower' onclick='javascript: document.getElementById("<?php echo "qty".$flowernum; ?>").value++; calculatePrice_wForm(); writeFPrice();' value='+'/>
	<input type='button' class="dec_button" name='subtract_flower' onclick='javascript: document.getElementById("<?php echo "qty".$flowernum; ?>").value--; calculatePrice_wForm(); writeFPrice();' value='-' />
</td>