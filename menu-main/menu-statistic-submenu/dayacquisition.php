<head>
    ...
    <script type="text/javascript">
		...
		function newAcquisition(date) {
			$.ajax({
				type: "GET",
				url: "menu-main/menu-statistic-submenu/acquisition/addAcquisition2.php?datum="+date,
				success: function(data){
					$('#newAcquisition').html(data);
...
<body>
    <div class="title">
        <span class="firstWord">Napi Beszerzési </span> Adatok
...
//		*/
		...
		echo '<div>
                ...
                <a href="?&page=statisztika&subpage=napibeszerzes&datum='.date("Ymd", strtotime($today) - 1 * 24 * 3600).'"><button type="button" class = "button" id = "newButton" >Elöző Nap</button></a>
                <a href="?&page=statisztika&subpage=napibeszerzes&datum='.date("Ymd", strtotime($today) + 1 * 24 * 3600).'"><button type="button" class = "button" id = "newButton" >Következő Nap</button></a>
				
                <button style="float:right;" type="button" class = "button" id = "newButton" onClick="newAcquisition('.$today.');">Új Beszerzés</button>
...
//		echo $other_incoming_query;
        ...
        while ($row = mysql_fetch_array($other_incoming_result)) {
			echo '<tr>';
				...
				echo '<td >
						<a onClick="delAcquisition('.$row["id"].')" href="#" style="background-image:url(http://www.nyilvantarto.koszorusarok.hu/img/icons/Delete-icon.png); background-repeat:no-repeat; width:32px; height: 32px;" class="button" ></a>					
					</td>';
