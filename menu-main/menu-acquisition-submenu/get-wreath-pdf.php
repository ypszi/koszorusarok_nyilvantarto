<?php
	...
	class PdfWithHeaderAndFooter extends FPDF
	{
		function Header() 	// Page header
		{
			...
			$this->Cell(0,0,'Beszerzés - Koszorú lista',0,0,'C'); 		// Title
			$this->Ln(13);			
		}
	...
	$pdf->Cell(23,0,$beginDate . " - " . $endDate,0,0,'L');
		...
		$query = "SELECT base_wreath_type.type AS type, base_wreath.id, base_wreath.size
		...
		$pdf->SetFont('Arial','',10);	//Betütípus beállítása
		$pdf->SetTextColor(137,165,146); 
		$pdf->Cell(50,0,"Típus",0,0,'L');
		$pdf->Cell(40,0,"Méret",0,0,'L');
		...
		while ($row = mysqli_fetch_assoc($result)) {
			if (isset($wreaths[$row["id"]])){
				...
				$pdf->Cell(40,0,$row["size"],0,0,'L');
				$pdf->Cell(30,0,$wreaths[$row["id"]] . ' db',0,0,'L');
