...
/*			$(".shopCheck").on("click", function(){
				...
				shop2 = $('#shop2').checked();
				shop3 = $('#shop3').checked();
				alert(shop1 + " " + shop2 + " " + shop3);
				$.ajax({
					type: "POST",
					url: "menu-main/menu-acquisition-submenu/acquisition.php",
					data: {
					shop1: shop1,
					shop2: shop2,
					shop3: shop3
...
<body>
	<div class="title">
	<span class="firstWord">Várható </span> beszerzések
...
//		echo $query;
		...
		echo '
			<div>
				<span style="font: normal 14px/16px Arial; color: #d93c76;">Koszorú alapok</span>
				...
				echo'
					<span><a href="/menu-main/menu-acquisition-submenu/get-wreath-pdf.php?min_date='.$beginDate.'&max_date='.$endDate.$link .'" target="_blank">Lista nyomtatása</a><span>
				</span>
			</div>
			<div style="width: 840px; padding: 15px; border-bottom: dotted 1px #cb361b;">
				<table>';
		...
		$query = "SELECT base_wreath_type.type, base_wreath.id, base_wreath.size
				...
				ORDER BY base_wreath_type.type;";

		$result = mysql_query($query) or die (mysql_error());		
	
		while ($row = mysql_fetch_assoc($result)) {
			if (isset($wreaths[$row["id"]])){
				echo '<tr><td width="250px">' .  $row["type"] . '</td><td width="200px">' . $row["size"] . '</td><td>' . $wreaths[$row["id"]]. ' db</td></tr>';
			}
		}
		echo '</table>
			</div>';
		...
		echo '<div style="padding-top: 20px;">
				<span style="font: normal 14px/16px Arial; color: #d93c76;">Virágok</span>
				<span class="print" style="float:right;">';
				...
				echo '<span><a href="/menu-main/menu-acquisition-submenu/get-flower-pdf.php?min_date='.$beginDate.'&max_date='.$endDate.$link .'" target="_blank">Lista nyomtatása</a><span>';
			echo'</span>
			</div>
		...
		$query = "SELECT *
		FROM flower 
		ORDER BY type ASC, color ASC;";

		$result = mysql_query($query) or die (mysql_error());
		
		while ($row = mysql_fetch_assoc($result)) {
			if (isset($flowers[$row["id"]])){
				echo '<tr><td width="250px">' .  $row["type"] . '</td><td width="200px">' . $row["color"] . '</td><td>' . $flowers[$row["id"]]. ' db</td></tr>';
			}
		}
		echo '</table>
			</div>';
