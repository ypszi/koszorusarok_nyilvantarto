...
<div  id="content" style="padding: 10px; margin-top:0px; background-color: #fff; height: 1250px; width:1040px;">

<div class="title">
	<span class="firstWord"><?php echo $shop[$_GET["shop"]]; ?></span> Naptár
...
/*		if (isset($_GET['week'])){
		...
		$last_day_in_week = date('Y-m-d',($time + 6*24*3600));
*/		
//		echo '<span class="next_week"></span>';		
		
		echo '<span class="next_week" style="float:right;"><a href="?page=naptar">Vissza a heti naptárba</a> | <a href="?page=naptar&subpage=bolt&shop='.$_GET["shop"].'&week='.($week-1).'">elöző</a> | <a href="?page=naptar&subpage=bolt&shop='.$_GET["shop"].'">aktualis hét</a> | <a href="?page=naptar&subpage=bolt&shop='.$_GET["shop"].'&week='.($week+1).'">következő</a></span>';
...
<?php
			...
			$query = "SELECT id, shop, shipment, paid, downprice,ritual_time, maker_shop
			   FROM orders 
			   WHERE WEEK(ritual_time,1)=". ($week) ." AND maker_shop = ".$_GET["shop"]." AND archive = 0
		...
		for ($hour = 7; $hour <= 20; $hour++) {
			...
			for ($day = 1; $day <= 7; $day++){
				if ($day != date("N") || $week != date("W")){
					echo '<td>';
					if (isset($order[$_GET["shop"]][$day][$hours[$hour]])){
						echo $order[$_GET["shop"]][$day][$hours[$hour]];
				}
